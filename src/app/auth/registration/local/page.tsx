...
export default function Page() {
  ...
  function validate(): boolean {
    ...
    if (!aadhaar.match(/^\d{12}$/)) errs.push("Aadhaar must be a 12 digit number");
  ...
  async function handleSendOtp() {
    ...
    try {
      const res = await fetch('/api/otp/send', {
        ...
        body: JSON.stringify({ mobile, purpose: 'register_local' }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        // Redirect to central OTP page to enter code
        router.push(`/auth/otp?mobile=${mobile}&purpose=register_local&next=${encodeURIComponent('/auth/registration/local')}`);
      } else {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      // Normally send data to server here. For now simulate success.
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          <h1 className="text-lg font-bold flex items-center gap-2">
            <span className="inline-block align-middle">
              <svg width="28" height="28" fill="none" viewBox="0 0 24 24">
                <path fill="currentColor" d="M12 2a7 7 0 0 1 7 7v3.382a2 2 0 0 0 .553 1.382l1.447 1.447A2 2 0 0 1 22 17.414V19a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3v-1.586a2 2 0 0 1 .553-1.382l1.447-1.447A2 2 0 0 0 5 12.382V9a7 7 0 0 1 7-7Zm0 2a5 5 0 0 0-5 5v3.382a4 4 0 0 1-1.106 2.764l-1.447 1.447A1 1 0 0 0 3 17.414V19a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-1.586a1 1 0 0 0-.447-.829l-1.447-1.447A4 4 0 0 1 17 12.382V9a5 5 0 0 0-5-5Z"/>
              </svg>
            </span>
      ...
      <main className="flex flex-1 flex-col items-center justify-start">
        <form
          ...
          className="bg-white rounded-lg shadow-lg p-8 w-full max-w-2xl mt-8 mb-8 flex flex-col gap-6 overflow-y-auto max-h-[calc(100vh-120px)]"
        >
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-blue-900">Local (Indian) Registration</h2>
            <Link href="/auth/registration/option" className="text-sm text-blue-700 hover:underline">Back to options</Link>
          </div>

          <p className="text-sm text-gray-600">Provide accurate details for safety and verification.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input className="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-400 placeholder:text-gray-700 placeholder:font-medium text-gray-700" value={fullName} onChange={(e) => setFullName(e.target.value)} required placeholder="As in government ID" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Date of birth *</label>
              <input type="date" className="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-400 placeholder:text-gray-700 placeholder:font-medium text-gray-700" value={dob} onChange={(e) => setDob(e.target.value)} required />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Gender</label>
              <select className="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-400 placeholder:text-gray-700 placeholder:font-medium text-gray-700" value={gender} onChange={(e) => setGender(e.target.value)}>
            ...
            <div>
              ...
              <input inputMode="numeric" pattern="\\d*" maxLength={12} className="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-400 placeholder:text-gray-700 placeholder:font-medium text-gray-700" value={aadhaar} onChange={(e) => setAadhaar(e.target.value.replace(/[^0-9]/g, "").slice(0, 12))} placeholder="12 digit Aadhaar" required />
            </div>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">Current / Residential address *</label>
              <input className="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-400 placeholder:text-gray-700 placeholder:font-medium text-gray-700" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="House / Street / City / State" required />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <input className="w-full px-3 py-2 border rounded focus:outline-none focus:ring focus:border-blue-400 placeholder:text-gray-700 placeholder:font-medium text-gray-700" value={hotelAddress} onChange={(e) => setHotelAddress(e.target.value)} placeholder="Hotel / Host address" />
            </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <label htmlFor="consent" className="text-sm text-gray-700">I consent to the collection and processing of my data for safety and verification purposes. I understand the <a className="text-blue-700 hover:underline" href="#">privacy policy</a>.</label>
          </div>
